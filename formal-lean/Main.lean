...
def printUOVSignatureScheme : IO Unit := do
  IO.println "==================================================="
  IO.println " UOVSignatureScheme — Complete UOV over GF(p)"
  IO.println "==================================================="
  IO.println ""
  IO.println "Adds the complete signature scheme on top of FiniteFieldUOV.lean:"
  IO.println "multivariate public key P = S ∘ F ∘ T, signing via trapdoor"
  IO.println "inversion (ZeroOilBlock → linear system over GFp p), verification,"
  IO.println "machine-checked correctness, and EUF-CMA security reduction."
  IO.println ""
  IO.println "§1    Key types"
  IO.println ""
  IO.println "  UOVMessage o p      : OilVec o p  (target vector in GF(p)^o)"
  IO.println "  UOVSignature o v p  : {oil : OilVec o p, vin : VinegarVec v p}"
  IO.println "  InvertibleFn α      : {fwd, inv, fwd_inv, inv_fwd} (explicit two-sided inverse)"
  IO.println "  UOVSecretKey o v p  : {F, hZero, S, T_o, T_v, hSolve}"
  IO.println "  UOVPublicKey o v p  : OilVec o p → VinegarVec v p → OilVec o p"
  IO.println ""
  IO.println "§2    Key generation"
  IO.println ""
  IO.println "  [1] uov_keygen        : UOVSecretKey → UOVPublicKey"
  IO.println "        P(oil, vin) = S(F(T_o(oil), T_v(vin)))."
  IO.println "  [2] uov_pk_decomposes : uov_keygen sk oil vin = S(F(T_o oil, T_v vin))"
  IO.println "        Definitional decomposition (rfl); used as a rewrite rule."
  IO.println ""
  IO.println "§3    Verification"
  IO.println ""
  IO.println "  [3] uov_verify          : UOVPublicKey → UOVSignature → UOVMessage → Bool"
  IO.println "        if P(sig.oil, sig.vin) = msg then true else false."
  IO.println "  [4] uov_verify_true_iff : uov_verify pk sig msg = true ↔ P(sig.oil,sig.vin) = msg"
  IO.println "        Biconditional specification of the Boolean verify function."
  IO.println ""
  IO.println "§4    Signing (trapdoor inversion)"
  IO.println ""
  IO.println "  [5] uov_sign            : UOVSecretKey → VinegarVec → UOVMessage → UOVSignature"
  IO.println "        Steps: vin' = T_v(vin_random); msg' = S⁻¹(msg);"
  IO.println "               solve F(oil', vin') = msg' (linear by ZeroOilBlock);"
  IO.println "               sig = (T_o⁻¹(oil'), vin_random)."
  IO.println "  [6] uov_sign_vin        : (uov_sign sk vin msg).vin = vin"
  IO.println "        Vinegar component is the chosen random vinegar (rfl)."
  IO.println "  [7] uov_sign_oil        : (uov_sign sk vin msg).oil = T_o⁻¹(choose(hSolve vin' msg'))"
  IO.println "        Oil component is T_o⁻¹ of the linear-system solution (rfl)."
  IO.println "  [8] uov_signing_equation: F(T_o(sig.oil), T_v(vin)) = S⁻¹(msg)"
  IO.println "        Core signing invariant; used in the correctness proof."
  IO.println "  [9] signing_equation_is_linear"
  IO.println "        F(a + c·b, T_v(vin)) = F(a,·) + c·F(b,·)  (ZeroOilBlock applied)"
  IO.println "        The signing equation is a linear system → O(o³) Gaussian elimination."
  IO.println ""
  IO.println "§5    Correctness"
  IO.println ""
  IO.println "  [10] uov_correctness  : uov_verify (uov_keygen sk) (uov_sign sk vin msg) msg = true"
  IO.println "         Proof: P(sig.oil, sig.vin) = S(F(oil', T_v(vin)))"
  IO.println "                                    = S(S⁻¹(msg)) = msg."
  IO.println "  [11] uov_sign_verifies : uov_keygen sk sig.oil sig.vin = msg"
  IO.println "         Propositional (non-Bool) form of correctness."
  IO.println "  [12] signature_solution_in_coset"
  IO.println "         Two oil inputs verifying the same (msg, vin) differ by a kernel element."
  IO.println "         F(T_o(oil1) − T_o(oil2), T_v(vin)) = 0."
  IO.println "         Source of signature randomness / unlinkability."
  IO.println ""
  IO.println "§6    EUF-CMA security reduction"
  IO.println ""
  IO.println "  [13] forger_is_mq_solver"
  IO.println "         Any EUF-CMA forger is an MQ solver (factor-1 reduction)."
  IO.println "         Proof: forging valid (oil, vin) for P is exactly inverting P."
  IO.println "  [14] uov_euf_cma_secure"
  IO.println "         ¬∃ forger, ∀ pk msg, uov_verify pk (forger pk msg) msg = true"
  IO.println "         Under MQ_gfp_hard, no forger exists."
  IO.println ""
  IO.println "Axiom: MQ_gfp_hard  (Multivariate Quadratic hardness over GF(p))."
  IO.println "       GF(p) analogue of MQ_hard from SignVerify.lean."
  IO.println ""
  IO.println "14 theorems — all machine-checked, zero sorry."
  IO.println ""
  IO.println "Proof chain for uov_correctness:"
  IO.println "  uov_correctness"
  IO.println "    → uov_signing_equation  (T_o.inv_fwd, Classical.choose_spec)"
  IO.println "    → uov_pk_decomposes     (rfl)"
  IO.println "    → S.inv_fwd, T_o.inv_fwd   (InvertibleFn fields)"
  IO.println "  No dependency on BalanceHypothesis axioms."
  IO.println "  Only computational axiom: MQ_gfp_hard (§6)."
  IO.println ""
  IO.println "GF(p) UOV complete scheme:"
  IO.println "  Keygen:  sk = (F, hZero, S, T_o, T_v, hSolve)"
  IO.println "           pk = P = S ∘ F ∘ (T_o, T_v)"
  IO.println "  Sign:    choose vin_random; solve F(·, T_v(vin)) = S⁻¹(msg) for oil';"
  IO.println "           sig = (T_o⁻¹(oil'), vin_random)"
  IO.println "  Verify:  check P(sig.oil, sig.vin) = msg"
  IO.println "  Correct: proven (uov_correctness) — no assumptions beyond sk structure"
  IO.println "  Secure:  EUF-CMA under MQ hardness (uov_euf_cma_secure)"
  IO.println ""
  IO.println "See UOVSignatureScheme.lean for full proof terms."
  IO.println ""
