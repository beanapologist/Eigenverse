...
def printChemicalBonds : IO Unit := do
  ...
  IO.println "  interactions.  Three required ingredients:"
  IO.println "    (A) A Hamiltonian with discrete eigenvalues below the dissociation threshold"
  IO.println "    (B) A variational principle: the minimum is attained by an explicit ground state"
  IO.println "    (C) Bond energy gap: bonded state is strictly below separated atoms"
  IO.println ""
  IO.println "§1    Molecular Hamiltonian  (Tunneling sector)"
  IO.println ""
  IO.println "  hamiltonianEigenvalue n hn = rydbergEnergy n hn = −1/n²  (Hartree a.u.)"
  IO.println "  separatedAtomEnergy = 0  (ionization threshold)"
  IO.println "  bondFormationEnergy = hamiltonianEigenvalue 1 − separatedAtomEnergy = −1"
  IO.println ""
  IO.println "  [1]  hamiltonian_ground_eigenvalue            : H(1) = −1 Hartree"
  IO.println "  [2]  hamiltonian_eigenvalues_negative         : H(n) < 0  for all n ≥ 1"
  IO.println "  [3]  hamiltonian_eigenvalues_below_dissociation : H(n) < separatedAtomEnergy"
  IO.println "         All eigenstates lie strictly below the dissociation threshold."
  IO.println "  [4]  hamiltonian_spectrum_discrete            : H(n) < H(n+1)  (strict ordering)"
  IO.println "         Discrete, non-degenerate spectrum — foundation for spectroscopy."
  IO.println ""
  IO.println "§2    Variational principle and bond energy  (variational lower bound)"
  IO.println ""
  IO.println "  [5]  bond_formation_negative_energy   : bondFormationEnergy < 0"
  IO.println "         ΔE = −1 − 0 = −1 Hartree < 0  ← bond formation releases energy"
  IO.println "  [6]  variational_minimum_attained     : ∃ n₀, ∀ n, H(n₀) ≤ H(n)"
  IO.println "         Explicit witness n₀ = 1; minimum EXISTS and is ATTAINED."
  IO.println "  [7]  ground_state_unique_minimum      : H(1) < H(n)  for n > 1"
  IO.println "         The minimum is unique — no alternative ground state."
  IO.println "  [8]  bonded_state_energy_gap          : H(1) < separatedAtomEnergy"
  IO.println "         Finite energy barrier of 1 Hartree separates bond from ionization."
  IO.println ""
  IO.println "§3    Orbital amplitude and bond formation  (Balance sector)"
  IO.println ""
  IO.println "  [9]  chem_bonding_amplitude_balance       : 2η² = 1"
  IO.println "         Two-electron orbital: each electron contributes η² = 1/2."
  IO.println "  [10] chem_bond_state_normalised            : η² + |μ·η|² = 1"
  IO.println "         Bond state has unit probability — quantum probability conservation."
  IO.println "  [11] chem_bond_maximum_coherence           : C(1) = 1"
  IO.println "         Symmetric bond achieves maximum coherence."
  IO.println "  [12] chem_off_balance_coherence_reduced    : C(r) < 1  for r ≠ 1, r ≥ 0"
  IO.println "         Asymmetric bonds are strictly less coherent (less stable)."
  IO.println "  [13] chem_orbital_balance_from_unit_circle : η² + η² = 1"
  IO.println "         Unit-circle constraint forces equal orbital amplitudes."
  IO.println ""
  IO.println "§4    Quantum-to-classical stability bridge  (Balance sector)"
  IO.println ""
  IO.println "  [14] chem_quantum_classical_bridge         : |Re(μ)| = Im(μ)"
  IO.println "         Quantum tunneling amplitude (Im=η) = classical funneling (|Re|=η)."
  IO.println "  [15] chem_bond_stability_zero_imbalance    : imbalance(μ) = 0"
  IO.println "         Zero imbalance is the formal stability criterion."
  IO.println "  [16] chem_bond_sign_duality                : Re(μ)·Im(μ) < 0"
  IO.println "         Quantum and classical are sign-dual at the bond equilibrium."
  IO.println ""
  ...
  IO.println "         (∀ n≥1, H(n) < separatedAtomEnergy)                   ← Hamiltonian"
  IO.println "         ∧ (∃ n₀, ∀ n, H(n₀) ≤ H(n))                          ← variational min"
  IO.println "         ∧ bondFormationEnergy < 0                              ← energy gap"
  IO.println "         ∧ 2η²=1  ∧  imbalance(μ)=0  ∧  mass conserved"
  ...
  IO.println ""
  IO.println "Hypothesis result: CONFIRMED."
  IO.println "  The atomic Hamiltonian has a discrete spectrum entirely below the"
  IO.println "  dissociation threshold (§1); the variational minimum is attained"
  IO.println "  by the explicit ground state n₀=1 (§2); the bonded state sits"
  IO.println "  1 Hartree below separated atoms (§2); the Balance bridge connects"
  IO.println "  the quantum minimum to classical stability (§§3–4); and the"
  IO.println "  resulting molecules carry conserved observable masses (§5)."
  IO.println ""
