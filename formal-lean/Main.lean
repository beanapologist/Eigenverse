...
def printChemicalBonds : IO Unit := do
  ...
  IO.println ""
  IO.println "  GAP 1 — Specialise A to the molecular Hamiltonian"
  IO.println "    def molecularHamiltonian (R : ℝ) : ℋ →L[ℂ] ℋ"
  IO.println "    (requires Kato-Rellich: Coulomb potential relatively bounded w.r.t. −Δ)"
  IO.println ""
  ...
  IO.println "    (requires spectral theorem for compact self-adjoint operators)"
  IO.println ""
  ...
  IO.println "    (requires Im⟨H_mol ψ, ψ⟩ → 0 as R → ∞; Re part remains negative but"
  IO.println "     the tunneling condition Im > 0 is eventually violated)"
  IO.println ""
