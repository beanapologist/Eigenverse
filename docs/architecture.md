...
## Foundational Principle: Funneling → Tunneling

The entire Eigenverse library is generated by two primitive interaction types:

**Funneling** narrows the infinite space of complex numbers down to a single
point — the critical eigenvalue μ = exp(i·3π/4) — through a cascade of
four axiom-driven funnels (energy conservation, directed balance, observer
sector selection, self-referential coherence closure).

**Tunneling** then propagates the structure anchored at μ outward across all
scales and domains via the palindrome symmetry C(r) = C(1/r), the Lyapunov
bridge C(exp λ) = sech λ, the 8-cycle phase orbit, and the six morphism
families proved in `Morphisms.lean`.

Every module in this repository is either a funneling proof (establishing
μ as the unique fixed point) or a tunneling proof (transporting Eigenverse
structure to a new domain).
...
│                                                                     │
│  ★ FUNNELING: BalanceHypothesis.lean → reality_unique → μ          │
│  ↓  TUNNELING: all remaining modules (coherence, orbit, wormhole,  │
│                morphisms, sector duality, temporal frustration …)   │
├─────────────────────────────────────────────────────────────────────┤
